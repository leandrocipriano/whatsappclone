...
export class Message extends Model {
    ...
    getViewElement(me = true){
                ...
                div.innerHTML = `<div class="_3_7SH _1ZPgd">
                    <div class="_1fnMt _2CORf">
                        ...
                        <div class="_3cMIj">
                            ...
                            <span class="PyPig message-file-type">${Format.fileType(this.fileType)}</span>
                            <span class="PyPig message-file-size">${Format.bytes(this.size)}</span>
                        </div>
                        <div class="_3Lj_s">
                            <div class="_1DZAH" role="button">
                                <span class="message-time">${Format.timeStampToTime(this.timeStamp)}</span>
        ...
        if(me){
            ...
            //Procura todas as divs com message-time e adiciona ao pai os status das mensagens
    ...
    static sendAudio(chatId, from, file, metadata, photo){
        ...
        return Message.send(chatId, 'audio', from, '').then(msgRef => {

            Message.upload(file, from).then(snapshot=>{
                   
                let downloadFile =  firebaseURL + snapshot.metadata.bucket + "/o/" + encodeURIComponent(snapshot.metadata.fullPath) + "?alt=media";
                
                msgRef.set({
                    content: downloadFile,
                    size: Format.bytes(file.size),
                    fileType: Format.fileType(file.type),
                    status: 'sent',
                    photo,
    ...
    static sendContact(chatId, from, contact){
        ...
        return Message.send(chatId, 'contact', from, contact);
    }
    ...
    static sendDocument(chatId, from, file, filePreview, info){
        ...
        Message.send(chatId, 'document', from, '').then(msgRef=>{
            ...
            Message.upload(file, from).then(snapshot=>{
                ...
                if(filePreview) 
                {
                    Message.upload(filePreview, from).then(snapshot2=>{
                        ...
                        let downloadFilePreview =  firebaseURL + snapshot2.metadata.bucket + "/o/" + encodeURIComponent(snapshot2.metadata.fullPath) + "?alt=media";
        ...
        return new Promise((success, failure)=>{
            ...
            Message.upload(file, from).then(snapshot=>{
                Message.send(
                    ...
                    'image', 
                    from, 
                    firebaseURL + snapshot.metadata.bucket + "/o/" + encodeURIComponent(snapshot.metadata.fullPath) + "?alt=media"
                ).then(()=>{
                    success();
                });
    ...
    static send(chatId, type, from, content){
        ...
        return new Promise((success, failure)=>{
            ...
            Message.getRef(chatId).add({
                ...
                //Retorna a mensagem que eu inseri
                docRef.set({
                    status:'sent'
                }, {
                    merge: true
    ...
    getStatusViewElement(){
        ...
        switch (this.status) {
            case 'wait':
                ...
                break;
            case 'sent':
                div.innerHTML = `<span data-icon="msg-check-light">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 15" width="16" height="15">
